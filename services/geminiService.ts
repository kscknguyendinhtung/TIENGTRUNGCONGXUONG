...
export const fetchFromScript = async (scriptUrl: string, user: string) => {
  ...
  try {
    ...
    if (text.trim().startsWith("<!DOCTYPE") || text.trim().startsWith("<html")) {
      console.error("Script returned HTML instead of JSON. Check Script permissions.");
      return null;
    }
...
export const extractVocabulary = async (input: { text?: string, imageBase64?: string }): Promise<any[]> => {
  ...
  if (input.imageBase64) {
    ...
    parts.push({ text: "OCR hình ảnh này và trích xuất từ vựng quan trọng." });
  } else if (input.text) {
...
export const analyzeImageAndExtractText = async (base64Images: string[]): Promise<SentenceAnalysis[]> => {
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview', 
    contents: { parts: [...imageParts, { text: "OCR và phân tích ngữ pháp. Phân loại từ vựng vào: Danh từ, Động từ, Tính từ, Mẫu câu, Sản xuất, Chất lượng, Nhân sự, Văn phòng, Khác." }] },
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.ARRAY,
...
export const generateMindmap = async (words: any[]): Promise<MindmapCategory[]> => {
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
    contents: `Phân loại từ vựng vào các nhóm logic: "Danh từ", "Động từ", "Tính từ", "Mẫu câu", "Sản xuất", "Chất lượng", "Nhân sự", "Văn phòng", "Khác". \n\n ${JSON.stringify(words)}`,
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.ARRAY,
