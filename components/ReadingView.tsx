...
export const ReadingView: React.FC<ReadingViewProps> = ({ currentUser, onDataChange }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Bóc tách từ vựng để đưa vào Script 2
      ...
      alert(`Đã nạp bài đọc và ${extractedWords.length} từ vựng mới.`);
    } catch (err) { alert("Lỗi quét ảnh."); } finally { setLoading(false); }
  };
    ...
    <div className="px-5 pb-28 pt-4 max-w-lg mx-auto bg-white min-h-screen">
      ...
      <div className="bg-slate-50 p-5 rounded-[28px] mb-6 border border-slate-100 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-200">
            ...
            <button onClick={() => { setIsBackgroundAudio(!isBackgroundAudio); toggleBackgroundMode(!isBackgroundAudio); }} className={`px-3 py-1.5 rounded-lg text-[8px] font-black transition-all ${isBackgroundAudio ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-500'}`}>{isBackgroundAudio ? 'BẬT' : 'TẮT'}</button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-8 bg-slate-100 p-1 rounded-2xl">
        ...
        <button onClick={() => setShowMastered(true)} className={`flex-1 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${showMastered ? 'bg-white shadow-sm text-green-600' : 'text-slate-400'}`}>ĐÃ THUỘC</button>
      </div>
      ...
      {selectedWord && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-6 z-[100]" onClick={() => setSelectedWord(null)}>
          <div className="bg-white p-8 rounded-[40px] shadow-2xl max-w-sm w-full text-center border-b-[8px] border-blue-600" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3"><button onClick={() => speakText(selectedWord.text, 'cn')} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px]">NGHE</button><button onClick={() => setSelectedWord(null)} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px]">ĐÓNG</button></div>
          </div>
        </div>
      )}
    </div>
